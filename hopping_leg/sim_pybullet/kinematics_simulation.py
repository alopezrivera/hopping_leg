...
sys.path.append("../state_estimation/")
from state_estimation import StateEstimation


class SimulationTracker():
    def __init__(self, simulation, se = None,  saveRecording=False, filename="../results/pybulletRecoridng.mp4"):
        ...
        Args:
            ...
            filename (str, optional): Filename and path for the simulation video. Has to end with ".mp4". Defaults to "../results/pybulletRecoridng.mp4".
        """
        if saveRecording:
    ...
    def write_csv(self,folder="../results"):
        """Write (almost?) all data in csv file

        Args:
            folder (str, optional): Location folder where to save data. Defaults to "../results".
        """
        ...
        with open(os.path.join(folder, 'results.csv'),"w+") as f:
    ...
    def trajectory_export(self,folder="../results"):
        ...
        for i in range(len(vars)):
            if vars[i]:
                write_names.append(names[i])
                write_vars.append(vars[i])    
        with open(os.path.join(folder, 'trajectoryexport.csv'),"w+") as f:
            # f.writelines("time,x,y,x_desired,y_desired,xd,yd,xd_desired,yd_desired,zdd,h,q1,q2,q1_desired,q2_desired,q1d,q2d,q1d_desired,q2d_desired,tau1,tau2,tau1_desired,tau2_desired,Fx,Fy,Fz,Fx_desired,Fy_desired,Fz_desired,basepos,basepos_desired\n")
            f.write(",".join(write_names)+"\n")
            for i in range(len(self.t)):
                # f.write(",".join([self.t[i],self.x[i],self.y[i],self.x_desired[i],self.y_desired[i],self.xd[i],self.yd[i],self.xd_desired[i],self.yd_desired[i],self.zdd[i],self.h[i],self.q1[i],self.q2[i],self.q1_desired[i],self.q2_desired[i],self.q1d[i],self.q2d[i],self.q1d_desired[i],self.q2d_desired[i],self.tau1,self.tau2[i],self.tau1_desired[i],self.tau2_desired[i],self.Fx[i],self.Fy[i],self.Fz[i],self.Fx_desired[i],self.Fy_desired[i],self.Fz_desired[i],self.basepos[i],self.basepos_desired[i]])+"\n")
    ...
    def create_plots(self, coordinates=True, trajectory=True,
                     ...
                     save_figures=False, folder="../results"):
        ...
        Args:
            ...
            folder (str, optional): Folder location where to save plots. Defaults to "../results".
        """
        ...
        if coordinates:
            ...
            if save_figures:
                plt.savefig(os.path.join(folder, 'position.png'))
                plt.close()
        if trajectory:
            ...
            plt.legend(loc='upper right')
            if save_figures:
                plt.savefig(os.path.join(folder, 'trajectory.png'))
                plt.close()
        if ee_velocity:
            ...
            if self.phase:
                ...
                ax.pcolorfast((self.t[0],self.t[-1]),ax.get_ylim(),np.array(self.phase)[np.newaxis],cmap="Greens", alpha=.3)
            if save_figures:
                plt.savefig(os.path.join(folder, 'velocity.png'))
                plt.close()
        if acceleration:
            if len(self.zdd) > 0:
                ...
                if self.phase:
                    ...
                    ax.pcolorfast((self.t[0],self.t[-1]),ax.get_ylim(),np.array(self.phase)[np.newaxis],cmap="Greens", alpha=.3)
                if save_figures:
                    plt.savefig(os.path.join(folder, 'acceleration.png'))
                    plt.close()
                ...
                if self.phase:
                    ...
                    ax.pcolorfast((self.t[0],self.t[-1]),ax.get_ylim(),np.array(self.phase)[np.newaxis],cmap="Greens", alpha=.3)
                if save_figures:
                    plt.savefig(os.path.join(folder, 'estimated_height.png'))
                    plt.close()
        if joint_angles:
            ...
            if self.phase:
                ...
                ax.pcolorfast((self.t[0],self.t[-1]),ax.get_ylim(),np.array(self.phase)[np.newaxis],cmap="Greens", alpha=.3)
            if save_figures:
                plt.savefig(os.path.join(folder, 'joint_angles.png'))
                plt.close()
        if joint_velocities:
            ...
            if self.phase:
                ...
                ax.pcolorfast((self.t[0],self.t[-1]),ax.get_ylim(),np.array(self.phase)[np.newaxis],cmap="Greens", alpha=.3)
            if save_figures:
                plt.savefig(os.path.join(folder, 'joint_velocities.png'))
                plt.close()
        if joint_efforts:
            ...
            if self.phase:
                ...
                ax.pcolorfast((self.t[0],self.t[-1]),ax.get_ylim(),np.array(self.phase)[np.newaxis],cmap="Greens", alpha=.3)
            if save_figures:
                plt.savefig(os.path.join(folder, 'joint_efforts.png'))
                plt.close()
        if ground_reaction_forces:
            ...
            if self.phase:
                ...
                ax.pcolorfast((self.t[0],self.t[-1]),ax.get_ylim(),np.array(self.phase)[np.newaxis],cmap="Greens", alpha=.3)
            if save_figures:
                plt.savefig(os.path.join(folder, 'reaction_forces.png'))
                plt.close()
        if base_position:
            ...
            if save_figures:
                plt.savefig(os.path.join(folder, 'base_position.png'))
                plt.close()
    ...
    def __init__(self, control_mode = 2, rail=True, drop_height=0.5, gravity=9.81, torque_limits=(6,6),long_leg = False, frequency = None):
        ...
        if self.rail:
            ...
            if long_leg: # old Rail
                urdfpath= "../model//with_rails/urdf/v7.urdf"
            else: # new Rail
                urdfpath = "../model/with_rails2/urdf/vertical_test.SLDASM.urdf"

            self.robot = p.loadURDF(
                urdfpath, [0, 0, 0], useFixedBase=1, physicsClientId=self.physicsClient)
        ...
        # no rail
            ...
            urdfpath = "../model/without_rails/urdf/v6_new_joint_limits.urdf"

            self.robot = p.loadURDF(urdfpath, [
        ...
        # setup variables for control mode:
