...
def get_control(controller, phase, state, effort, desired_height):
    ...
    return shoulder_pos_des, elbow_pos_des, shoulder_vel_des, elbow_vel_des, tau1,tau2, phase
...
height_factor_vec = np.zeros(numSteps)
...
if val == 'y' or val == 'Y':
    ...
    for i in range(numSteps):
        ...
        shoulder_pos, shoulder_vel, shoulder_tau = motor_shoulder.send_rad_command(shoulder_pos_des, shoulder_vel_des, Kp_shoulder, Kd_shoulder, shoulder_tau_des)
        elbow_pos, elbow_vel, elbow_tau = motor_elbow.send_rad_command(elbow_pos_des, elbow_vel_des, Kp_elbow, Kd_elbow, elbow_tau_des)

        state = [shoulder_pos, elbow_pos, shoulder_vel, elbow_vel]

        phase_vec[i] = phase_dict[current_phase]

        shoulder_pos_des, elbow_pos_des, shoulder_vel_des, elbow_vel_des, shoulder_tau_des, elbow_tau_des, current_phase = get_control(controller, current_phase, state, np.array([shoulder_tau, elbow_tau]), desired_height)
