...
path_to_submechamisms = os.environ["AUTOPROJ_CURRENT_ROOT"] + "/../underactuated-robotics/hopping_leg/model/with_rails/urdf/submechanisms_railfixed.yml"
...
for i in range(numStepsToInit):
    ...
    # Compute Inverse Dynamics using HyRoDyn
    robot.y = np.array([shoulder_pos_des, elbow_pos_des])
    robot.yd = np.array([shoulder_vel_des, elbow_vel_des])
    robot.ydd = np.array([shoulder_acc_des, elbow_acc_des])
    robot.calculate_inverse_dynamics()
    tau = robot.Tau_actuated.flatten()
    shoulder_tau_des = tau[0]
    elbow_tau_des = tau[1]
...
np.savetxt("desired_data.csv", measured_csv_data, delimiter=',', header="time,shoulder_pos,shoulder_vel,shoulder_torque,elbow_pos,elbow_vel,elbow_torque", comments="")
