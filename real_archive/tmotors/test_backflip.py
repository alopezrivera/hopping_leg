...
def get_control(controller, phase, state, effort):
    ...
    if phase == "TOUCHDOWN":
        ...
        # change
        shoulder_pos_des, elbow_pos_des, shoulder_vel_des, elbow_vel_des, tau1, tau2 = controller.cartesian_stiffness(state, p_d = [x_des+0.12,y_des], pd_d = [0.0,0.0], Kpc = (1000,2000), Kdc=(10,10), f_ff = [100.0,0.0], knee_direction=1)
...
L2 = 0.2
g = -9.81
plant = HopperPlant(link_length=[L1, L2], gravity=g, torque_limits = [12,12])
...
motor_shoulder = CanMotorController(can_socket='can0', motor_id=motor_shoulder_id, motor_type='AK80_6_V2', socket_timeout=0.5)
...
if val == 'y' or val == 'Y':
    ...
    for i in range(numSteps):
        ...
        if(flight_counter==2):
            print("ready to backflip")
            shoulder_pos, shoulder_vel, shoulder_tau = motor_shoulder.send_rad_command(shoulder_pos_des+6.2831, 0, Kp_shoulder, Kd_shoulder, 0.0)
            elbow_pos, elbow_vel, elbow_tau = motor_elbow.send_rad_command(elbow_pos_des, 0, Kp_elbow, Kd_elbow, 0.0)
            break
