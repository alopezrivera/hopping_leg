...
# Settings
des_time_step = 0.00125                               # Desired time step that you want to upscale the data to  
search_dir = '/home/dfki.uni-bremen.de/mboukheddimi/DEVEL/DFKI/OC_UNDERACTUATED/hopping_leg/Controllers/OC_FDDP/results/withoutRAILPZ/2hops/logs' # Set search directory
log_ident = 'log'                                   # Set log file identifier
...
'''
# Check that joint space already has been filled
if alreadyFilled == False: 
    print('Error: Please first fill up logJointSpace.csv by executing $python3 addFixedJointValuesToLogs>')
    sys.exit(0)
'''
...
for fileName in logsToUpscale:
    ...
    for t in np.arange(0, total_time+des_time_step, des_time_step):
        data_interp.append([np.asscalar(interpolator[col](t)) for col in cols])
